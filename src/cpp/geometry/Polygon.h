...
namespace rir
{
	...
	bool polygonIsRect(const std::vector<_Point<T>> &_p, _Rect<T> *rect)
	{
		...
		if (rect)
			*rect = _Rect<T>(posx[0], posy[0], posx[1], posy[1]);
		return true;
	}
	...
	*/
	...
	struct OrientedRect
	{
		...
		OrientedRect(const PolygonF &bp = PolygonF(), const PolygonF &hp = PolygonF(), const PointF &c = PointF(),
					 double w = 0, double h = 0, double wa = 0, double ha = 0) : boundingPoints(bp), hullPoints(hp), center(c), width(w), height(h), widthAngle(wa), heightAngle(ha) {}
	};
