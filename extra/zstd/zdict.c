...
unsigned ZDICT_getDictID(const void *dictBuffer, size_t dictSize)
{
    if (dictSize < 8)
        return 0;
    if (MEM_readLE32(dictBuffer) != ZSTD_DICT_MAGIC)
        return 0;
    ...
    const void *buffer, U32 minRatio, U32 notificationLevel)
{
    ...
    if ((MEM_read16(b + pos + 0) == MEM_read16(b + pos + 2)) || (MEM_read16(b + pos + 1) == MEM_read16(b + pos + 3)) || (MEM_read16(b + pos + 2) == MEM_read16(b + pos + 4)))
    {
        ...
        U32 u, e = 6;
        while (MEM_read16(b + pos + e) == u16)
            e += 2;
        if (b[pos + e] == b[pos + e - 1])
            e++;
        for (u = 1; u < e; u++)
            doneMarks[pos + u] = 1;
        return solution;
    ...
    ZDICT_params_t params)
{
    ...
    if (maxDictSize <= g_provision_entropySize + g_min_fast_dictContent)
    {
        free(dictList);
        return ERROR(dstSize_tooSmall);
    }
