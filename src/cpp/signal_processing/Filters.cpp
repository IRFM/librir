...
#pragma omp parallel for num_threads(4)
	...
	*/
	...
	{
		if (vector_count == 0) return std::vector<double>();
		...
		if (s & ResampleIntersection) {
			...
			for (size_t i = 0; i < vector_count; ++i) {
				if (end < start) { //init
					...
					if (vectors[i][vector_sizes[i] - 1] < start) return res; //null intersection
					else if (vectors[i][0] > end) return res; //null intersection
	...
	std::vector<double> resampleSignal(const double* sample_x, const double* sample_y, size_t size, const double* times, size_t times_size, int s, double padds)
	{
		...
		for (size_t t = 0; t < times_size; ++t)
		{
			...
			if (iters_x == ends_x) {
				if (s & ResamplePadd0) res[t] = padds;
				else res[t] = sample_y[size - 1];
				continue;
			}
			...
			//we are before the sample
				...
				//in between 2 values
					...
					if (s & ResampleInterpolation) {
						...
						if (time - *prev_x < samp_x - time) res[t] = *prev_y;
						else  res[t] = samp_y;
					}
				}
			}
				...
				if (iters_x != ends_x) {
					if (*iters_x == time) {
						...
						if (s & ResampleInterpolation) {
							...
							//take the closest value
							if (time - *prev_x < *iters_x - time) res[t] = *prev_y;
							else  res[t] = *iters_y;
						}
					}
				}
