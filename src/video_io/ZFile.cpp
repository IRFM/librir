...
void * z_open_memory_read(void * mem, uint64_t size)
{
	...
	//read headers
	memcpy(&res->bheader, res->mem + res->pos, sizeof(res->bheader)); res->pos += sizeof(res->bheader);
	memcpy(&res->btrigger, res->mem + res->pos, sizeof(res->btrigger)); res->pos += sizeof(res->btrigger);
	...
	if (res->btrigger.data_size_x > 0 && res->btrigger.data_size_x < 2000 && res->btrigger.data_size_y > 0 && res->btrigger.data_size_y < 2000 && res->btrigger.rate > 0 && res->btrigger.rate < 1000)
	{
		...
		for (size_t i = 0; i < res->btrigger.samples; ++i)
		{
			...
			res->positions[i] = pos;
			memcpy(&timestamp, res->mem + pos, sizeof(timestamp)); pos += sizeof(timestamp);
			memcpy(&fsize, res->mem + pos, sizeof(fsize)); pos += sizeof(fsize);
			res->timestamps[i] = timestamp;
...
void * z_open_memory_write(void * mem, uint64_t size, int width, int height, int rate, int method, int clevel )
{
	...
	memcpy(res->mem + res->pos, &res->bheader, sizeof(res->bheader)); res->pos += sizeof(res->bheader);
	memcpy(res->mem + res->pos, &res->btrigger, sizeof(res->btrigger)); res->pos += sizeof(res->btrigger);
...
uint64_t z_close_file(void * file)
{
	...
	if (f && !f->readOnly)
	{
		...
		for (size_t i = 0; i < f->btrigger.samples; ++i) {
			...
			if (!is_little_endian()) p = swap_int64(p);
			memcpy(poss, &p, 8);
			poss += 8;
		}
...
int z_write_image(void * file, const unsigned short * img, int64_t timestamp)
{
	...
	else if (f->mem)
	{
		...
		memcpy(f->mem + f->pos, &timestamp, sizeof(timestamp)); f->pos += sizeof(timestamp);
		memcpy(f->mem + f->pos, &csize, sizeof(csize)); f->pos += sizeof(csize);
		memcpy(f->mem + f->pos, f->buffer.data(), csize); f->pos += csize;
		f->btrigger.samples++;
		return 0;
	}
...
int z_read_image(void * file, int pos, unsigned short * img, int64_t * timestamp )
{
	...
	else if (f->mem)
	{
		...
		memcpy(&time, f->mem + loc, sizeof(time)); loc += sizeof(time);
		if (timestamp) *timestamp = time;
		memcpy(&fsize, f->mem + loc, sizeof(fsize)); loc += sizeof(fsize);
