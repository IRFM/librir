...
int IRFileLoader::findFileType(char * buf, PCR_HEADER * infos, int64_t * start_images, int64_t * start_time, int * frame_count)
{
	...
	//test PCR file
	if (pcr_header.Bits == 16 && ((abs(pcr_header.TransfertSize - (pcr_header.X * pcr_header.Y * 2)) < 2000) /*|| pcr_header.Frequency == 50 || pcr_header.Frequency == 25*/)
		&& pcr_header.X > 0 && pcr_header.Y > 0 && pcr_header.X < 2000 && pcr_header.Y < 2000)
	{
		/*if (((abs(pcr_header.TransfertSize - (pcr_header.X * pcr_header.Y * 2)) > 2000)))
		pcr_header.TransfertSize = pcr_header.X * pcr_header.Y * 2;*/ //wrong transfer size, correct it
		if (infos)
			*infos = pcr_header;
		if (start_images)
		...
		pcr_enc.X > 0 && pcr_enc.Y > 0 && pcr_enc.X < 1000 && pcr_enc.Y < 1000)
	{
		...
		pcr_enc.TransfertSize = pcr_enc.X * pcr_enc.Y * 2;*/ //wrong transfer size, correct it
		if (infos)
			*infos = pcr_enc;
		if (start_images)
...
static BinFile * bin_open_file_from_file_reader(const char * filename, void * reader)
{
	...
	else if (f->type == BIN_FILE_Z_COMPRESSED)
	{
		...
		if( t0 > 28000 && t0 < 32000) {
			...
			if (!(f->times.front() < -1000000000 || f->times.back() > 1000000000)) {//time already in ns
				for (unsigned int i = 0; i < f->count; ++i)
...
static void bin_close_file(BinFile * f)
{
	...
	if(f->other)
		f->other->close();
	f->other.reset();
	if (f->file) destroyFileReader(f->file);
	delete f;
}
...
static int bin_read_image(BinFile * f, int pos, unsigned short * img, int64_t * timestamp, int calib=0)
{
	...
	if (f->type == BIN_FILE_OTHER) {
		if (timestamp) *timestamp = f->times[pos];
...
const std::map<std::string, std::string> & IRFileLoader::globalAttributes() const
{
	...
	const std::map<std::string, std::string> & res = (m_data->type == BIN_FILE_H264) ?
		((BinFile*)m_data->file)->h264.globalAttributes() : 
		(m_data->type == BIN_FILE_HCC ? ((BinFile*)m_data->file)->hcc.globalAttributes() : 
			(m_data->type == BIN_FILE_OTHER ? m_data->file->other->globalAttributes() : empty_map));
