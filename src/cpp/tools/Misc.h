...
namespace rir
{
	...
	*/
	...
	{
		const union { unsigned i; unsigned char c[4]; } one = { 1 };   /* don't use static : performance detrimental  */
		return one.c[0];
	}
...
#pragma GCC diagnostic pop
	...
	{
		const union { uint32_t i; uint8_t c[4]; } one = { 1 };   /* don't use static : performance detrimental  */
		return one.c[0];
	}
	...
	public:
		...
		T readFloat(bool* ok = NULL) {
...

			{
				...
				if (max_read > 32) max_read = 32;
				if (file) fread(buff + 1, max_read, 1, file);
				else memcpy(buff + 1, string + pos, max_read);
				char* start = buff;
	...
	*/
	...
	Array2D<T> readFileFast(FileFloatStream& fin, std::string* error = NULL)
	{
		...
		if (!fin.isOpen()) {
			if (error) *error = "Cannot read stream: not open";
			return Array2D<T>();
		}
		bool ok;
		...
		if (!is_ascii)
		{
			...
			if (!ok) {
				...
				if (!ok) {
					if (error) *error = "Cannot interpret file format";
					return Array2D<T>();
				}
			}
		}
...

		{
			...
			while (true) {
				...
				if (res.width && (int)res.width != width) {
					if (error) *error = "The file does not contain a matrix ";
					return Array2D<T>();
				}
	...
	*/
	...
	Array2D<T> readFileFast(const char* filename, std::string* error = NULL)
	{
		...
		//open file
		if (!fin.isOpen()) {
			if (error) *error = "Cannot read file " + std::string(filename);
			return Array2D<T>();
		}
