	...
	*/
	...
	public:
		...
		bool setEmissivities(const float* emi, size_t size) {
			...
			for (size_t i = 0; i < _s; ++i) m_invEmissivities[i] = 1.f / emi[i];
			//fill missing values
			for (size_t i = _s - 1; i < m_invEmissivities.size(); ++i) m_invEmissivities[i] = 1.f;
			m_globalEmi = 0;
			return true;
		}
		bool setInvEmissivities(const float* inv_emi, size_t size) {
			...
			if ((size_t)m_invEmissivities.size() < _s)
				_s = (size_t)m_invEmissivities.size();
			for (size_t i = 0; i < _s; ++i) m_invEmissivities[i] = inv_emi[i];
			//fill missing values
			for (size_t i = _s - 1; i < m_invEmissivities.size(); ++i) m_invEmissivities[i] = 1.f;
			m_globalEmi = 0;
			return true;
