...
#define AVSEEK_END SEEK_END
	...
	*/
	...
	{
		void* opaque = nullptr;
		file_infos infos = nullptr;
		read_chunk read = nullptr;
	...
	*/
	TOOLS_EXPORT FileReaderPtr createFileReader(FileAccess&& access);
	...
	*/
	TOOLS_EXPORT int readFile(FileReader* file_reader, void* buf, int buf_size);
	static inline int readFile(FileReaderPtr& file_reader, void* buf, int buf_size) { return readFile(file_reader.get(), buf, buf_size); }

	TOOLS_EXPORT int readFile2(FileReader* file_reader, uint8_t* outbuf, int buf_size);
	static inline int readFile2(FileReaderPtr& file_reader, uint8_t* outbuf, int buf_size) { return readFile2(file_reader.get(), outbuf, buf_size); }
	/**
	Seek file reader to given position. Returns -1 on error, or the new position.
	*/
	TOOLS_EXPORT int64_t seekFile(FileReader* file_reader, int64_t pos, int whence);
	static inline int64_t seekFile(FileReaderPtr& file_reader, int64_t pos, int whence) { return seekFile(file_reader.get(), pos, whence); }
	/**
	Return current file reader position.
	*/
	TOOLS_EXPORT int64_t posFile(FileReader* file_reader);
	static inline int64_t posFile(FileReaderPtr& file_reader) { return posFile(file_reader.get()); }
	/**
	Get file size from file reader object.
	*/
	TOOLS_EXPORT int64_t fileSize(FileReader* file_reader);
	static inline int64_t fileSize(FileReaderPtr& file_reader) { return fileSize(file_reader.get()); }
