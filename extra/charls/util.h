...
namespace charls
{
	...
	inline int32_t sign(const int32_t n) noexcept
	{
		return (n >> (int32_t_bit_count - 1)) | 1;
	}

	inline int32_t bit_wise_sign(const int32_t i) noexcept
	{
	...
	{
		return 2 * (bits_per_pixel + (8 > bits_per_pixel ? 8 : bits_per_pixel)); // std::max(8, bits_per_pixel));
	}

	template <typename SampleType>
	struct triplet
	{
		triplet() noexcept : v1{0},
							 v2{0},
							 v3{0}
		{
		}

		triplet(int32_t x1, int32_t x2, int32_t x3) noexcept : v1(static_cast<SampleType>(x1)),
	...
	struct quad final : triplet<SampleType>
	{
		...
		quad() noexcept : triplet<SampleType>(),
						  v4{0}
		{
		}
		...
		MSVC_WARNING_SUPPRESS(26495) // false warning that v4 is uninitialized [VS 2017 15.9.4]
		quad(triplet<SampleType> triplet_value, int32_t alpha) noexcept : triplet<SampleType>(triplet_value),
