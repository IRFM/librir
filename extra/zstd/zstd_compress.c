...
size_t ZSTD_estimateCCtxSize(ZSTD_compressionParameters cParams)
{
    ...
    size_t const neededSpace = tableSpace + (256 * sizeof(U32)) /* huffTable */ + tokenSpace + (((cParams.strategy == ZSTD_btopt) || (cParams.strategy == ZSTD_btopt2)) ? optSpace : 0);
...
{
    return (param1.cParams.hashLog == param2.cParams.hashLog) & (param1.cParams.chainLog == param2.cParams.chainLog) & (param1.cParams.strategy == param2.cParams.strategy) & ((param1.cParams.searchLength == 3) == (param2.cParams.searchLength == 3));
}
                                      ...
                                      ZSTD_compResetPolicy_e const crp)
{
...

    {
        ...
        {
            size_t const optSpace = ((MaxML + 1) + (MaxLL + 1) + (MaxOff + 1) + (1 << Litbits)) * sizeof(U32) + (ZSTD_OPT_NUM + 1) * (sizeof(ZSTD_match_t) + sizeof(ZSTD_optimal_t));
            size_t const neededSpace = tableSpace + (256 * sizeof(U32)) /* huffTable */ + tokenSpace + (((params.cParams.strategy == ZSTD_btopt) || (params.cParams.strategy == ZSTD_btopt2)) ? optSpace : 0);
                                                    ...
                                                    const U32 mls)
{
    ...
    { /* < instead of <=, because (ip+1) */
        ...
        if ((((U32)((dictLimit - 1) - repIndex) >= 3) /* intentional underflow */ & (repIndex > lowestIndex)) && (MEM_read32(repMatch) == MEM_read32(ip + 1)))
        {
                                           ...
                                           const U32 mls)
{
    ...
    { /* < instead of <=, because repcode check at (ip+1) */
        ...
        if ((offset_1 > 0) & (MEM_read32(ip + 1 - offset_1) == MEM_read32(ip + 1)))
        { /* note : by construction, offset_1 <= current */
            mLength = ZSTD_count(ip + 1 + 4, ip + 1 + 4 - offset_1, iend) + 4;
            ip++;
            ZSTD_storeSeq(seqStorePtr, ip - anchor, anchor, 0, mLength - MINMATCH);
        }
                                    ...
                                    ZSTD_parameters params, U64 pledgedSrcSize, U32 dictID)
{
    ...
    U32 const fcsCode = params.fParams.contentSizeFlag ? (pledgedSrcSize >= 256) + (pledgedSrcSize >= 65536 + 256) + (pledgedSrcSize >= 0xFFFFFFFFU) : /* 0-3 */
                            0;
                                             ...
                                             U32 frame, U32 lastFrameChunk)
{
    ...
    if (srcSize)
    {
        size_t const cSize = frame ? ZSTD_compress_generic(cctx, dst, dstCapacity, src, srcSize, lastFrameChunk) : ZSTD_compressBlock_internal(cctx, dst, dstCapacity, src, srcSize);
        if (ZSTD_isError(cSize))
            return cSize;
        return cSize + fhSize;
    }
...
static size_t ZSTD_loadDictEntropyStats(ZSTD_CCtx *cctx, const void *dict, size_t dictSize)
{
    ...
    if (cctx->rep[0] == 0 || cctx->rep[0] >= dictSize)
        return ERROR(dictionary_corrupted);
    cctx->rep[1] = MEM_readLE32(dictPtr + 4);
    if (cctx->rep[1] == 0 || cctx->rep[1] >= dictSize)
        return ERROR(dictionary_corrupted);
    cctx->rep[2] = MEM_readLE32(dictPtr + 8);
    if (cctx->rep[2] == 0 || cctx->rep[2] >= dictSize)
        return ERROR(dictionary_corrupted);
    dictPtr += 12;
    ...
    while (someMoreWork)
    {
        ...
        case zcss_load:
            ...
            /* compress current block (note : this stage cannot be stopped in the middle) */
            {
                ...
                cSize = (flush == zsf_end) ? ZSTD_compressEnd(zcs->cctx, cDst, oSize, zcs->inBuff + zcs->inToCompress, iSize) : ZSTD_compressContinue(zcs->cctx, cDst, oSize, zcs->inBuff + zcs->inToCompress, iSize);
                if (ZSTD_isError(cSize))
                    return cSize;
                if (flush == zsf_end)
                    zcs->frameEnded = 1;
                ...
                zcs->stage = zcss_flush; /* fall through */ /* pass-through to flush stage */
                /* fall through */
            }
...
size_t ZSTD_endStream(ZSTD_CStream *zcs, ZSTD_outBuffer *output)
{
    ...
    if (zcs->stage != zcss_final)
    {
        ...
        zcs->outBuffContentSize = !notEnded ? 0 : ZSTD_compressEnd(zcs->cctx, zcs->outBuff, zcs->outBuffSize, NULL, 0); /* write epilogue, including final empty block, into outBuff */
    }
...
ZSTD_compressionParameters ZSTD_getCParams(int compressionLevel, unsigned long long srcSize, size_t dictSize)
{
    ...
    if (MEM_32bits())
    { /* auto-correction, for 32-bits mode */
