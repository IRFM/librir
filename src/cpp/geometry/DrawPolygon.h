	...
	{                            \
		...
		static inline void fillBottomFlatTriangle(const Point &v1, const Point &v2, const Point &v3, Array2DView<T> &out, T color)
		{
			...
			if (start_y < 0 && end_y < 0)
				return;
			else if (start_y >= out.height && end_y >= out.height)
				return;
			if (start_y < 0)
				start_y = 0;
			if (end_y >= out.height)
	...
	*/
	...
	size_t fillPolygonFunctor(const Point *pts, size_t size, Functor &f, const Rect &rect)
	{
		...
		if (!rect.isNull())
		{
			...
			if (pr.xmin < rect.xmin)
				pr.xmin = rect.xmin;
			if (pr.xmax > rect.xmax)
				pr.xmax = rect.xmax;
			if (pr.ymin < rect.ymin)
				pr.ymin = rect.ymin;
			if (pr.ymax > rect.ymax)
				pr.ymax = rect.ymax;
		}
		...
		for (pixel_y = pr.ymin; pixel_y < pr.ymax; pixel_y++)
		{
			...
			if (pixel_y > pr.ymin)
			{
				for (i = 0; i < nr; i++)
				{
					if (((pts[i].y() < pixel_y && pts[j].y() >= pixel_y) ||
						 (pts[j].y() < pixel_y && pts[i].y() >= pixel_y)) &&
						pts[j].y() != pts[i].y())
					{
			...
			for (i = 0; i < nodes; i += 2)
			{
				...
				if (node_x[i + 1] >= pr.xmin)
				{
					if (node_x[i] < pr.xmin)
						node_x[i] = pr.xmin;
					if (node_x[i + 1] >= pr.xmax)
						node_x[i + 1] = pr.xmax - 1;
