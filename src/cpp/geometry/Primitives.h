...
namespace rir
{
	...
	*/
	...
	template <class T>
	inline _Point<T> operator+(const _Point<T> &p1, const _Point<T> &p2) { return _Point<T>(p1.x() + p2.x(), p1.y() + p2.y()); }
	template <class T>
	inline _Point<T> operator+(const _Point<T> &p1, T p2) { return _Point<T>(p1.x() + p2, p1.y() + p2); }
	template <class T>
	inline _Point<T> operator+(T p1, const _Point<T> &p2) { return _Point<T>(p1 + p2.x(), p1 + p2.y()); }
	template <class T>
	inline _Point<T> &operator+=(_Point<T> &p1, const _Point<T> &p2)
	{
		p1.rx() += p2.x();
		p1.ry() += p2.y();
		return p1;
	...
	template <class T>
	inline _Point<T> &operator+=(_Point<T> &p1, T p2)
	{
		p1.rx() += p2;
		p1.ry() += p2;
		return p1;
	...
	struct _Rect
	{
		...
		_Rect normalized() const
		{
			_Rect r = *this;
			if (r.xmax < r.xmin)
				std::swap(r.xmax, r.xmin);
			if (r.ymax < r.ymin)
				std::swap(r.ymax, r.ymin);
			return r;
		}
