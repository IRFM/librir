...
{
	if(strcmp(preset,"ultrafast") == 0) return 0;
	if(strcmp(preset,"superfast") == 0) return 1;
	if(strcmp(preset,"veryfast") == 0) return 2;
	if(strcmp(preset,"faster") == 0) return 3;
	if(strcmp(preset,"fast") == 0) return 4;
	if(strcmp(preset,"medium") == 0) return 5;
	if(strcmp(preset,"slow") == 0) return 6;
	if(strcmp(preset,"slower") == 0) return 7;
	return 0;
}
...
bool H264_Saver::addImageLossyWithCamera(const unsigned short * img_DL, int64_t timestamp_ns, const std::map<std::string, std::string> & attributes)
{
	...
	if (m_data->attributes.size() == 0) {
		...
		if (m_data->subtractMin || m_data->subtractLocalMin) {
			...
			int s = m_data->width*m_data->stop_lossy_height;
			for (int i = 0; i < s; ++i)
				if (m_data->tmp[i] < m_data->min) m_data->min = m_data->tmp[i];
			//subtract min
#pragma omp parallel for num_threads(threads)
			for (int i = 0; i < s; ++i) {
				if (m_data->tmp[i] < m_data->min) m_data->tmp[i] = 0;
				else m_data->tmp[i] -= m_data->min;
			}
	...
	if (m_data->subtractMin || m_data->subtractLocalMin) {
		...
		if (m_data->subtractLocalMin) {
			...
			int s = m_data->width * m_data->stop_lossy_height;
			for (int i = 0; i < s; ++i)
				if (m_data->tmpT[i] < m_data->min) m_data->min = m_data->tmpT[i];
			if (min < m_data->min)
				m_data->min = min;
			m_data->localMins.push_back(m_data->min);
...
#pragma omp parallel for num_threads(threads)
	...
	if (highError < 0)highError = 0;
	if (lowError < highError)lowError = highError;
