...
void ZSTDMT_compressChunk(void *jobDescription)
{
    ...
    if (job->cdict)
    { /* should only happen for first segment */
    ...
    if (!job->firstChunk)
    { /* flush and overwrite frame header when it's not first segment */
    ...
    job->cSize = (job->lastChunk) ? ZSTD_compressEnd(job->cctx, dstBuff.start, dstBuff.size, src, job->srcSize) : ZSTD_compressContinue(job->cctx, dstBuff.start, dstBuff.size, src, job->srcSize);
...
ZSTDMT_CCtx *ZSTDMT_createCCtx(unsigned nbThreads)
{
    ...
    if ((nbThreads < 1) | (nbThreads > ZSTDMT_NBTHREADS_MAX))
        return NULL;
    ...
    if (!cctx->factory | !cctx->buffPool | !cctx->cctxPool)
    { /* one object was not created */
...

    {
        ...
        for (chunkID = 0; chunkID < nbChunks; chunkID++)
        {
            ...
            mtctx->jobs[chunkID].srcStart = NULL;
            {
                ...
                if (chunkID)
                { /* note : chunk 0 is already written directly into dst */
                    if (!error)
                                          ...
                                          ZSTD_parameters params, unsigned long long pledgedSrcSize)
{
    ...
    if (zcs->nbThreads == 1)
        return ZSTD_initCStream_advanced(zcs->cstream, dict, dictSize, params, pledgedSrcSize);
    if (zcs->allJobsCompleted == 0)
    { /* previous job not correctly finished */
    ...
    if (zcs->inBuff.buffer.start == NULL)
        return ERROR(memory_allocation);
    zcs->inBuff.filled = 0;
    zcs->dictSize = 0;
    zcs->doneJobID = 0;
...
{
    if (zcs->nbThreads == 1)
        return ZSTD_resetCStream(zcs->cstream, pledgedSrcSize);
...
static size_t ZSTDMT_createCompressionJob(ZSTDMT_CCtx *zcs, size_t srcSize, unsigned endFrame)
{
    ...
    if (!endFrame)
    {
        ...
        zcs->inBuff.buffer = ZSTDMT_getBuffer(zcs->buffPool, zcs->inBuffSize);
        if (zcs->inBuff.buffer.start == NULL)
        { /* not enough memory to allocate next input buffer */
            zcs->jobs[jobID].jobCompleted = 1;
            zcs->nextJobID++;
            ZSTDMT_waitForAllJobsCompleted(zcs);
            ZSTDMT_releaseAllJobResources(zcs);
...
static size_t ZSTDMT_flushNextJob(ZSTDMT_CCtx *zcs, ZSTD_outBuffer *output, unsigned blockToFlush)
{
    ...
    while (zcs->jobs[wJobID].jobCompleted == 0)
    {
        ...
        if (!blockToFlush)
        {
            pthread_mutex_unlock(&zcs->jobCompleted_mutex);
            return 0;
        }                                                                     /* nothing ready to be flushed => skip */
    ...
    /* compression job completed : output can be flushed */
    {
        ...
        if (!job.jobScanned)
        {
            ...
            if (zcs->params.fParams.checksumFlag)
            {
                ...
                if (zcs->frameEnded && (zcs->doneJobID + 1 == zcs->nextJobID))
                { /* write checksum at end of last section */
        ...
        if (job.dstFlushed == job.cSize)
        { /* output buffer fully flushed => move to next one */
        ...
        if (job.cSize > job.dstFlushed)
            return (job.cSize - job.dstFlushed);
        if (zcs->doneJobID < zcs->nextJobID)
            return 1;                            /* still some buffer to flush */
...
size_t ZSTDMT_compressStream(ZSTDMT_CCtx *zcs, ZSTD_outBuffer *output, ZSTD_inBuffer *input)
{
    ...
    if (zcs->frameEnded)
        return ERROR(stage_wrong); /* current frame being ended. Only flush is allowed. Restart with init */
    if (zcs->nbThreads == 1)
        return ZSTD_compressStream(zcs->cstream, output, input);
    ...
    { /* avoid overwriting job round buffer */
...
static size_t ZSTDMT_flushStream_internal(ZSTDMT_CCtx *zcs, ZSTD_outBuffer *output, unsigned endFrame)
{
    ...
    if (((srcSize > 0) || (endFrame && !zcs->frameEnded)) && (zcs->nextJobID <= zcs->doneJobID + zcs->jobIDMask))
    {
...
{
    if (zcs->nbThreads == 1)
        return ZSTD_flushStream(zcs->cstream, output);
...
{
    if (zcs->nbThreads == 1)
        return ZSTD_endStream(zcs->cstream, output);
