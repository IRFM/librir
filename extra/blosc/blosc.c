...
static const char *clibcode_to_clibname(int clibcode)
{
  ...
  if (clibcode == BLOSC_LZ4_LIB)
    return BLOSC_LZ4_LIBNAME;
  if (clibcode == BLOSC_SNAPPY_LIB)
    return BLOSC_SNAPPY_LIBNAME;
  if (clibcode == BLOSC_ZLIB_LIB)
    return BLOSC_ZLIB_LIBNAME;
  if (clibcode == BLOSC_ZSTD_LIB)
    return BLOSC_ZSTD_LIBNAME;
                   ...
                   uint8_t *tmp2)
{
  ...
  {
    /* Validate src_offset */
    if (src_offset < 0 || src_offset > compressedsize - (int)sizeof(int32_t))
    {
      return -1;
    }
